    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {filteredPublications.map(pub => (
          <div key={pub.id} className="bg-gray-800/50 p-6 rounded-lg border border-gray-700/50 backdrop-blur-sm hover:bg-gray-800/70 transition-all duration-300">
            {/* Publication Media */}
                ...
                <div className="mb-4">
                  {media.type === 'image' && (
                    <button
                      ...
                      className="w-full h-48 rounded-lg overflow-hidden border-2 border-violet-500/30 shadow-lg shadow-violet-500/10 focus:outline-none focus:ring-2 focus:ring-violet-500/50"
                    >
                      <RobustImage
                  ...
                  {media.type === 'video' && (
                    <button
                      onClick={() => onOpenModal?.(media, media.title || pub.title)}
                      className="w-full h-48 rounded-lg overflow-hidden border-2 border-pink-500/30 bg-gray-800 flex items-center justify-center shadow-lg shadow-pink-500/10 focus:outline-none focus:ring-2 focus:ring-pink-500/50"
                    >
                  ...
                  {(media.type === 'pdf' || media.type === 'ppt' || media.type === 'pptx') && (
                    <button
                      ...
                      className="w-full h-48 rounded-lg border-2 border-purple-500/30 bg-gray-800 flex items-center justify-center shadow-lg shadow-purple-500/10 focus:outline-none focus:ring-2 focus:ring-purple-500/50"
                    >
